...
tweetAuto(User,Message,0)->
    ...
    {project4, server_node()} ! {tweetupd, Tweetmp1, Newalltweets},
...
start_server() ->
    ...
    register(project4, spawn(project4, server, [[]])),
    register(fetch, spawn(project4, datafetcher, [])).
...
server_logon(From, Name, User_List) ->
    ...
    case lists:keymember(Name, 2, User_List) of
        true ->
            ...
            From ! {project4, stop, user_exists_at_other_node},  %reject logon
            User_List;
        false ->
            From ! {project4, logged_on},
...
server_follow(From, To, User_List) ->
    ...
    case lists:keysearch(From, 1, User_List) of
        false ->
            From ! {project4, stop, you_are_not_logged_on};
        {value, {From, Name}} ->
...
server_follow(From, Name, To, User_List) ->
    ...
    case lists:keysearch(To, 2, User_List) of
        false ->
            From ! {project4, receiver_not_found};
        {value, {ToPid, To}} ->
...
reg(Username, Password) ->
    ...
    io:fwrite(" ~p", [Hash]),
    {project4, server_node()} ! {register, self(), "reg", Username,Hash}.
                ...
                Booluser == true ->
                    ...
                    if 
                        Hash1 == Hashed_pass ->
                            register(
                                mess_client, 
                                spawn(project4, client, [server_node(), Name, Name])
                            );
                        true ->
                            io:fwrite("Joined")
                    end;
...
client(Server_Node, Name, Myname) ->
    {project4, Server_Node} ! {self(), logon, Name},
    await_result(),
...
client(Server_Node, Myname) ->
    ...
    receive
        logoff ->
            {project4, Server_Node} ! {self(), logoff},
            exit(normal);
        {message_to, ToName, Message} ->
            {project4, Server_Node} ! {self(), message_to, ToName, Message},
            await_result();
        {message_from, FromName, Message} ->
            io:format("Message from ~p: ~p~n", [FromName, Message]),

            {project4, Server_Node} ! {updlstmsg, Message, Myname};
        {follow_to, ToName} -> 
            {project4, Server_Node} ! {self(), follow_to, ToName},
            await_result();
        {follow_from, FromName} ->
            ...
            FollowersMap3 = maps:merge(FollowersMap, FollowersMap2),
            {project4, Server_Node} ! {update, FollowersMap3};
...
await_result() ->
    receive
        ...
        {project4, stop, Why} -> % Stop the client 
            io:format("~p~n", [Why]),
            exit(normal);

        {project4, What} ->  % Normal response
            io:format("~p~n", [What])
    end.